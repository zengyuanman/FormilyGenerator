...
let widgets = [];
const widgetsObj = {};
defaultSettings.forEach((m) => {
  widgets = [...widgets, ...m.widgets];
});
widgets.forEach((m) => {
  widgetsObj[m.name] = m;
});
...
export function combineSchema(propsSchema = {}, uiSchema = {}) {
  ...
  const newList = propList.map((p) => {
    ...
    const isObj = (type === 'object' || type === 'void') && properties;
...
export function flattenSchema(schema, name = '#', parent, result = {}) {
  ...
  const children = [];
  const isObj = ['object', 'void'].includes(_schema.type) && _schema.properties;
  const isList =
  ...
  if (_schema.type) {
    ...
    if (
      widgetsObj[_schema['y-setting-name']] &&
      widgetsObj[_schema['y-setting-name']].setting
    ) {
      result[name].setting = widgetsObj[_schema['y-setting-name']].setting;
    }
  }
  return result;
}
    ...
    if (item.children.length > 0) {
      item.children.forEach((child) => {
        ...
        if (
          schema.type === 'array' &&
          schema.items &&
          (schema.items.type === 'object' || schema.type === 'void')
        ) {
      ...
      if (
        schema.type === 'array' &&
        schema.items &&
        (schema.items.type === 'object' || schema.items.type === 'void') &&
        !schema.items.properties
      ) {
        schema.items.properties = {};
      }
...
export const addItem = ({
  ...
  if ((_selected && _selected[0] === '0') || _selected === '#') {
    ...
    try {
      ...
      newId = _selected === '#' ? `#/grid/${name}` : `${oldId}/${name}`;
      if (!fixedName) {
        newId += `_${nanoid(6)}`;
      }
      if (_selected === '#') {
...
export const dataToFlatten = (flatten, data) => {
  ...
  Object.entries(flatten).forEach(([id]) => {
    const branchData = getDataById(data, id);
    // TODO
    flatten[id].data = { ...flatten[id].data, ...branchData };
  });
  return flatten;
};
...
function toKey(value) {
  ...
  const result = `${value}`;
  return result === '0' && 1 / value === -window.Infinity ? '-0' : result;
}
...
export const oldSchemaToNew = (schema) => {
  ...
  if (schema && (schema.type === 'object' || schema.type === 'void')) {
    return schema;
  }
  return {
    type: 'object',
